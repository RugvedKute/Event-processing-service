...
- **Production Patterns**: Separation of concerns between ingestion and processing
- **Monitoring Ready**: Structured logging with event tracing
...
1. **BullMQ Dashboard**
   ...
   - Visualize job queues, retries, and failures

2. **Metrics (Prometheus)**

   - Job processing duration
   - Retry counts per event type
   - Queue depth and lag
   - Worker utilization

3. **Distributed Tracing**

   - Propagate trace IDs through Kafka headers
   - Track events from ingestion to completion

4. **Dead Letter Queue (DLQ)**

   - Route permanently failed jobs to a DLQ topic
...
**Symptom:** Consumer logs show connection errors
...
```bash
docker compose logs kafka-consumer | grep -i "validation"
```
...
# List all keys (in Redis CLI)
KEYS *

# Inspect a specific job
HGETALL bull:process-event:evt-001
```
